...
export default function Sidebar({
    ...
    type: string;
    lang: { overview?: string; security?: string; profile?: string; language?: string; users?: string; posts?: string; logs?: string };
}) {
    const pathname = usePathname();

    const sidebar = [
        { label: lang.overview, path: '/account', target: null },
        { label: lang.security, path: '/account/security', target: 'security' },
        { label: lang.profile, path: '/account/profile', target: 'profile' },
        { label: lang.language, path: '/account/language', target: 'language' },
    ];

    const admin = [
        { label: lang.users, path: '/account/admin/users', target: 'users' },
        { label: lang.posts, path: '/account/admin/posts', target: 'posts' },
        { label: lang.logs, path: '/account/admin/logs', target: 'logs' },
    ];

    return type === 'normal' ? (
        <>
            <ul className={o.normal}>
                {sidebar.map((list) => (
                    <li key={list.target}>
                        <Link href={list.path} className={list.path === pathname ? o.active : undefined}>
