        ...
        <div className={o.content}>
                ...
                <aside className={o.info}>
                    ...
                    <div className={o.meta}>
                        <div className={o.time}>
                            {'Published on '}
                            <time>{new Date(blog[0].createdAt).toLocaleDateString(user?.language || 'en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</time>
                        </div>
                        <div className={o.footer}>
                            By <Image src={blog[0].authorAvatar} alt={"User's avatar"} width={16} height={16} /> {blog[0].authorUsername}
                        </div>
                    </div>
                </aside>
            </div>
            ...
            <ul className={o.articles}>
                {blog.slice(1).map((post) => (
                    <li key={post.id}>
                        <Link href={post.title.toLowerCase().split(' ').join('-') + '-' + post.id.split('-')[post.id.split('-').length - 1]} title={post.title}>
                            ...
                            <div className={o.info}>
                                ...
                                <div className={o.footer}>
                                    By <Image src={post.authorAvatar} alt={"User's avatar"} width={16} height={16} /> {post.authorUsername}
                                </div>
                            </div>
                        </Link>
                    </li>
